...
export default function RoastGameboard({
  ...
  const [pendingCard, setPendingCard] = useState<string | null>(null);
  const [showTargetModal, setShowTargetModal] = useState(false);
  // Tracks the card currently being dragged so opponent avatars (and the
  // table, for untargeted cards) know what to do on drop. Cleared on
  // dragend regardless of whether the drop succeeded, so a card released
  // outside any drop zone never leaves the UI thinking a drag is in flight.
  ...
  function handleCardDragStart(cardId: string) {
    if (!isMyTurn || !me || me.isEliminated || !hasValidPlay(cardId)) return;
  ...
  // Untargeted cards (shields, self-buffs, group roast) can be dropped
  // anywhere on the table itself rather than on a specific opponent.
  function handleTableDrop(e: React.DragEvent) {
    e.preventDefault();
    if (!draggedCard) return;
    if (requiresTarget(draggedCard)) return; // needs handleOpponentDrop instead
    handlePlayCard(draggedCard);
    setDraggedCard(null);
    setDragOverTarget(null);
  }

  function handleOpponentDragOver(e: React.DragEvent, targetUserId: string) {
    e.preventDefault();
    if (dragOverTarget !== targetUserId) setDragOverTarget(targetUserId);
  }

  function handleOpponentDrop(e: React.DragEvent, target: Player) {
    e.preventDefault();
    setDragOverTarget(null);
    if (!draggedCard) return;
    ...
    const isValid = getValidTargets(cardId).some(
      (p) => p.userId === target.userId,
    );
    if (!isValid) {
      toast.error("🚫 Can't play that card on them right now!");
      return;
    }
    handlePlayCard(cardId, target.userId);
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col relative z-10 overflow-hidden">
        ...
        <div className="flex justify-center gap-4 pt-4 pb-2 px-4 flex-wrap">
          {otherPlayers.map((p) => {
            const isDropTarget = !!draggedCard && !p.isEliminated;
            const isHoveredDrop = dragOverTarget === p.userId;
            return (
              <div
                key={p.userId}
                onDragOver={
                  isDropTarget
                    ? (e) => handleOpponentDragOver(e, p.userId)
                    : undefined
                }
                onDragLeave={() =>
                  setDragOverTarget((cur) => (cur === p.userId ? null : cur))
                }
                onDrop={
                  isDropTarget ? (e) => handleOpponentDrop(e, p) : undefined
                }
                className={`flex flex-col items-center gap-1.5 rounded-2xl transition-all ${
                  p.isEliminated ? "opacity-40" : ""
                } ${
                  isHoveredDrop
                    ? "ring-2 ring-red-400 bg-red-500/10 scale-105"
                    : isDropTarget
        ...
        {/* Centre play area */}
        <div
          className={`flex-1 flex flex-col items-center justify-center gap-3 px-4 rounded-2xl transition-colors ${
            draggedCard && !requiresTarget(draggedCard)
              ? "bg-white/5 ring-1 ring-white/10"
              : ""
          }`}
          onDragOver={(e) => {
            if (draggedCard && !requiresTarget(draggedCard)) e.preventDefault();
          }}
